...
class CVoxel {
	...
	public:
		...
		CVoxel(const ObjIterator& begin, const ObjIterator& end); 
		...
		inline int contained_elements(const ObjIterator& begin, const ObjIterator& end) const; 
		inline bool intersects_with_vector(const CVector3D& vector) const; 
		inline int sort_and_count_contained(const ObjIterator& begin, const ObjIterator& end) const;
...
class CKDNode {
	private:
		...
		ObjIterator m_begin;
		ObjIterator m_end;
		inline CKDNode* MakeLeaf(const ObjIterator& begin, const ObjIterator& end);
	...
	public:
		...
		inline CKDNode* build(const ObjIterator& begin, const ObjIterator& end, const CVoxel& voxel, int depth);
